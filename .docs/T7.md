# T7 — Email Subscriptions & Notifications
...
2. **API routes:**
   - `/api/subscriptions` → create/update user subscription
   - `/api/notifications/send-digest` → send batch emails by topic
3. **Email template example:**
   ```ts
   const html = `
     <h2>Fresh ideas from Pain2Gain</h2>
     <ul>
       ${ideas.map(i => `<li><b>${i.title}</b> — ${i.elevatorPitch}</li>`).join('')}
     </ul>
